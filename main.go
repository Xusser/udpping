...
var (
	...
	alwaysNewConnection bool = false
)
...
func init() {
	...
	flag.BoolVar(&alwaysNewConnection, "n", false, "Always use new connection")
...
func client() {
	...
	go func() {
		...
		for {
			...
			if alwaysNewConnection {
				nc, err := net.DialUDP("udp", nil, addr)
				if err != nil {
					log.WithError(err).Fatalf("Fail to dial to %s", addr.String())
				}
				c = nc
				log.Tracef("Using new connection from %s", nc.LocalAddr().String())
			}
		}
	}()
